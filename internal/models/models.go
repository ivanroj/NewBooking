...
type Workspace struct {
    ...
    Name   string `db:"name" json:"name"`
    // Additional fields like coordinates, capacity, etc. can be added later
}

type User struct {
    ID        int64  `db:"id" json:"id"`
    TelegramID string `db:"telegram_id" json:"telegram_id"`
    Role      string `db:"role" json:"role"` // "student" or "admin"
...
type Booking struct {
    ...
    Status      string `db:"status" json:"status"` // e.g., "active", "canceled"
}
